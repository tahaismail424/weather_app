...
const UI = (function () {
  ...
  function displayMain() {
    ...
    let subtitle = createQuick(
      "h2",
      "Please enter city information to " + "get weather information",
      content
    );
  ...
  function makeForm() {
    function makeInput(type, form) {
      ...
      if (type === "state")
        prompt.textContent = "State name (if in US), select N/A if not in US: ";
      ...
      if (type === "city") {
        ...
        search.setAttribute(
          "pattern",
          data.locationData.states.join("|") + "|N/A"
        );
  ...
  function displayWeatherData(weather) {
    ...
    let timeDisplay = createQuick(
      "h2",
      `${currentTime.hour}:${currentTime.minutes} ${currentTime.ampm}`,
      displayContent
    );
    ...
    let riseTime = createQuick(
      "p",
      `${weather.sunrise.hour}:${weather.sunrise.minutes} ${weather.sunrise.ampm}`,
      sunriseBox
    );
    ...
    let setTime = createQuick(
      "p",
      `${weather.sunset.hour}:${weather.sunset.minutes} ${weather.sunset.ampm}`,
      sunsetBox
    );
    ...
    let actualNum = createQuick(
      "th",
      `${Math.round((weather.temp - 273.15) * 1.8 + 32)}°F`,
      numbers
    );
    let feelsNum = createQuick(
      "th",
      `${Math.round((weather.feelsLike - 273.15) * 1.8 + 32)}°F`,
      numbers
    );
    let minNum = createQuick(
      "th",
      `${Math.round((weather.min - 273.15) * 1.8 + 32)}°F`,
      numbers
    );
    let maxNum = createQuick(
      "th",
      `${Math.round((weather.max - 273.15) * 1.8 + 32)}°F`,
      numbers
    );
    ...
    function changeTempUnit() {
      if (currentFahr) {
        ...
        actualNum.textContent = `${Math.round(
          (weather.temp - 273.15) * 1.8 + 32
        )}°F`;
        feelsNum.textContent = `${Math.round(
          (weather.feelsLike - 273.15) * 1.8 + 32
        )}°F`;
        minNum.textContent = `${Math.round(
          (weather.min - 273.15) * 1.8 + 32
        )}°F`;
        maxNum.textContent = `${Math.round(
          (weather.max - 273.15) * 1.8 + 32
        )}°F`;

...
const data = (function() {
    ...
    function matchID(formData) {
        ...
        let ID;
        let altName = locationData.cityAlts[locationData.cities.indexOf(formData[0])];
        let stateID = locationData.stateCodes[locationData.states.indexOf(formData[1])];
        ...
        if (formData[0] && formData[1] !== 'N/A' && formData[2]) {
            ...
            for (let entry of weatherKeys) {
                if (entry.country === 'US' && entry.name === formData[0]
                && entry.state === stateID) ID = entry.id;
                else if (entry.country === 'US' && entry.name === altName
                && entry.state === stateID) ID = entry.id
            }
        } else if (formData[0] && formData[2] && formData[1] === 'N/A') {

            let country_ID = locationData.countryCodes[locationData.countries.indexOf(formData[2])];

            for (let entry2 of weatherKeys) {

                if (entry2.country === country_ID && entry2.name === formData[0]) ID = entry2.id;
                else if (entry2.country === country_ID && entry2.name === altName) ID = entry2.id;
            }
        }
    ...
    function WeatherData(city, country, temp, max, min, feelsLike, 
        sunset, sunrise, timezone, weather, sub, src) {
    ...
    function getWeatherData(formData) {
        ...
        return fetch(`https://api.openweathermap.org/data/2.5/weather?id=${city_ID}&appid=37ea418809aa56c31726ace0173d68da`
        , { mode: 'cors'})
        ...
        .then(response => {
            ...
            let weatherData = new WeatherData(response.name, 
                response.sys.country, response.main.temp, response.main.temp_max,
                response.main.temp_min, response.main.feels_like, new Date(response.sys.sunset * 1000),
                new Date(response.sys.sunrise * 1000), '', response.weather[0].main, response.weather[0].description,
    ...
    function getTimeZone(latitude, longitude, weatherData) {
        ...
        return fetch(`http://api.timezonedb.com/v2.1/get-time-zone?key=O0TVTNRS9DFL&format=json&by=position&lat=${latitude}&lng=${longitude}`,
        { mode: 'cors'})
        .then(response => response.json())
        .then(response => {
            ...
            //edit sundata
            weatherData.sunrise = getTime(weatherData.timezone, weatherData.sunrise);
    ...
    function getTime(timezone, time) {
        ...
        if (!Math.floor(timeData.minutes / 10)) timeData.minutes = '0' + timeData.minutes;
